...
def affine_forward(x, w, b):
  ...
  out = reshaped_x.dot(w) + b[:, np.newaxis].T
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    x_hat = x - sample_mean[:, np.newaxis].T
    x_hat /= np.sqrt((sample_var[:, np.newaxis].T + eps))

    out = gamma*x_hat + beta

    cache = (x, gamma, x_hat, sample_var, sample_mean, eps)
    ...
    out = x - running_mean[:, np.newaxis].T
    out /= np.sqrt((running_var[:, np.newaxis].T + eps))
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  x, gamma, x_hat, var, mu, eps = cache
  ...
  dx_hat = np.multiply(gamma, dout)

  dmu = -1.0/np.sqrt(var + eps)*np.sum(dx_hat, axis=0)
  da = np.multiply(1/np.sqrt(var+eps), dx_hat)
  de = -.5*1.0/np.power(var+eps, 1.5)*(x - mu)*dx_hat
  dvar = np.sum(de, axis=0)

  dx = da + 2.0*(x-mu)/N*dvar + 1.0/N*dmu
