...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    # ================================================================ #
      ...
      for j in np.arange(num_classes):
        if j != y[i]:
          temp_loss += max(0, 1 + self.W[j].dot(X[i]) - self.W[y[i]].dot(X[i]))
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
      ...
      for j in np.arange(num_classes):
        z = 1 + self.W[j].dot(X[i]) - self.W[y[i]].dot(X[i])
        
        if j != y[i]:
          temp_loss += max(0, z)
          grad[j] += X[i] if z > 0 else 0
          grad[y[i]] += -1*X[i] if z > 0 else 0
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    loss_mat = self.W.dot(X.T)
    loss_mat = loss_mat - loss_mat[y,np.arange(loss_mat.shape[1])] + 1
    loss_mat[y, np.arange(loss_mat.shape[1])] = 0
    loss_mat[loss_mat < 0] = 0
    loss = loss_mat.sum()/X.shape[0]
    ...
    rsum = np.sum(indicator, axis=0)
    indicator[y, np.arange(loss_mat.shape[1])] = -rsum
    grad = indicator.dot(X)/X.shape[0]
