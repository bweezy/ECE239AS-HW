...
def rmsprop(w, dw, config=None):
  ...
  a = dr*a + (1 - dr) * np.square(dw)

  next_w = w - lr*np.multiply(1/(np.sqrt(a)+eps),dw)  
...
def adam(w, dw, config=None):
  ...
  v = b1*v + (1-b1)*dw
  a = b2*a + (1-b2)*np.square(dw)

  v_mod = 1/(1-np.power(b1, t))*v
  a_mod = 1/(1-np.power(b2, t))*a

  next_w = w - lr*np.multiply(1/(np.sqrt(a_mod)+eps),v_mod)
