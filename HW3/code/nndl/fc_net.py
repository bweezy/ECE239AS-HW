...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    loss = np.sum(log_sums - y_terms)/num_examples + .5*self.reg*np.sum(W1*W1) + .5*self.reg*np.sum(W2*W2)

    d_scores = e_scores/sums[:,np.newaxis]
    d_scores[np.arange(num_examples),y] -= 1
    d_scores = d_scores.T/num_examples

    dx2, dw2, db2 = affine_backward(d_scores.T, cache2)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in np.arange(self.num_layers-1):
      temp_score, temp_cache = affine_relu_forward(layer_scores[i], self.params['W{}'.format(i+1)], self.params['b{}'.format(i+1)])
      caches.append(temp_cache)
      layer_scores.append(temp_score)

    temp_score, temp_cache = affine_forward(layer_scores[self.num_layers-1], self.params['W{}'.format(self.num_layers)], self.params['b{}'.format(self.num_layers)])
    caches.append(temp_cache)
    layer_scores.append(temp_score)
    ...
    for i in np.arange(self.num_layers-2, -1,-1):
      dx, dw, db = affine_relu_backward(d_scores, caches[i])
      grads['W{}'.format(i+1)] = dw + self.reg*self.params['W{}'.format(i+1)]
      grads['b{}'.format(i+1)] = db
      d_scores = dx

    #dx2, dw2, db2 = affine_relu_backward(d_scores, cache[i])
   # dx1, dw1, db1 = affine_relu_backward(dx2, cache1)
