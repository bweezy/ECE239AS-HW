...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    H1 = W1.dot(X.T) + b1[:, np.newaxis]
    H1[H1<0] = 0
    scores = W2.dot(H1) + b2[:,np.newaxis]
    scores = scores.T
    ...
    loss = np.sum(log_sums - y_terms)/num_examples + .5*reg*np.sum(W1*W1) + .5*reg*np.sum(W2*W2)
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
    ...
    b2_grad = np.sum(d_scores,axis=1)
    W2_grad = d_scores.dot(H1.T)

    r_grad = W2.T.dot(d_scores)
    r_grad[H1<=0] = 0

    b1_grad = np.sum(r_grad,axis=1)
    W1_grad = r_grad.dot(X)

    grads['b1'] = b1_grad
    grads['W1'] = W1_grad + reg*W1

    grads['b2'] = b2_grad
    grads['W2'] = W2_grad + reg*W2
